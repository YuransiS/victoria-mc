...
function getLocalUsers(): MinicourseUser[] {
  ...
  const data = localStorage.getItem('victoria_mc_minicourse_users');
  if (!data) {
    localStorage.setItem('victoria_mc_minicourse_users', JSON.stringify(DEFAULT_USERS));
  ...
  try {
    ...
    if (cleaned.length !== parsed.length) {
      localStorage.setItem('victoria_mc_minicourse_users', JSON.stringify(cleaned));
      return cleaned;
    }
    return parsed;
  } catch (e) {
...
function saveLocalUsers(users: MinicourseUser[]) {
  if (typeof window !== 'undefined') {
    localStorage.setItem('victoria_mc_minicourse_users', JSON.stringify(users));
  }
}

function getLocalProgress(): MinicourseProgress[] {
  if (typeof window === 'undefined') return DEFAULT_PROGRESS;
  const data = localStorage.getItem('victoria_mc_minicourse_progress');
  if (!data) {
    localStorage.setItem('victoria_mc_minicourse_progress', JSON.stringify(DEFAULT_PROGRESS));
  ...
  try {
    ...
    if (cleaned.length !== parsed.length) {
      localStorage.setItem('victoria_mc_minicourse_progress', JSON.stringify(cleaned));
      return cleaned;
    }
    return parsed;
  } catch (e) {
...
function saveLocalProgress(progress: MinicourseProgress[]) {
  if (typeof window !== 'undefined') {
    localStorage.setItem('victoria_mc_minicourse_progress', JSON.stringify(progress));
  }
}
...
function getLocalLessonsConfig(): MinicourseLessonConfig[] {
  ...
  const data = localStorage.getItem('victoria_mc_minicourse_lessons_config');
  if (!data) {
    localStorage.setItem('victoria_mc_minicourse_lessons_config', JSON.stringify(DEFAULT_LESSONS_CONFIG));
...
function saveLocalLessonsConfig(config: MinicourseLessonConfig[]) {
  if (typeof window !== 'undefined') {
    localStorage.setItem('victoria_mc_minicourse_lessons_config', JSON.stringify(config));
  }
}
...
export async function getLessonsConfig(): Promise<MinicourseLessonConfig[]> {
  if (IS_MOCK_MODE) {
    ...
    const { data, error } = await supabase!
      .from('victoria_mc_minicourse_lessons_config')
      .select('*')
...
function getLocalPrizeCodes(): MinicoursePrizeCode[] {
  ...
  const data = localStorage.getItem('victoria_mc_minicourse_prize_codes');
  if (!data) return [];
  try {
...
function saveLocalPrizeCodes(codes: MinicoursePrizeCode[]): void {
  if (typeof window !== 'undefined') {
    localStorage.setItem('victoria_mc_minicourse_prize_codes', JSON.stringify(codes));
  }
}
...
export async function getPrizeCodes(): Promise<MinicoursePrizeCode[]> {
  if (IS_MOCK_MODE) {
    ...
    const { data, error } = await supabase!
      ...
      .select(`
        *,
        victoria_mc_minicourse_users(name, telegram)
      `)
      .order('created_at', { ascending: false });
    ...
    return (data || []).map(row => {
      const u = row.victoria_mc_minicourse_users;
      return {
        code: row.code,
...
export async function getGiftTokens(): Promise<GiftTokenItem[]> {
  if (IS_MOCK_MODE) {
    if (typeof window === 'undefined') return [];
    return JSON.parse(localStorage.getItem('victoria_mc_minicourse_gift_tokens') || '[]');
  } else {
    const { data, error } = await supabase!
...
export async function generateGiftToken(): Promise<GiftTokenItem> {
  ...
  if (IS_MOCK_MODE) {
    ...
    const tokens = JSON.parse(localStorage.getItem('victoria_mc_minicourse_gift_tokens') || '[]');
    ...
    localStorage.setItem('victoria_mc_minicourse_gift_tokens', JSON.stringify(tokens));
    return newItem;
  } else {
    const { data, error } = await supabase!
      .from('victoria_mc_minicourse_gift_tokens')
...
export async function getBotTemplates(): Promise<BotMessageTemplate[]> {
  if (IS_MOCK_MODE) {
    if (typeof window === 'undefined') return [];
    const raw = JSON.parse(localStorage.getItem('victoria_mc_minicourse_bot_templates') || '[]');
    return raw.map((t: any) => ({ ...t, message_text: normalizeMessageText(t.message_text) }));
  }

  // 1. Fetch saved templates from database
  const { data: dbTemplates, error } = await supabase!
    .from('victoria_mc_minicourse_bot_templates')
    .select('*')
...
export async function saveBotTemplate(template: BotMessageTemplate): Promise<void> {
  ...
  if (IS_MOCK_MODE) {
    if (typeof window === 'undefined') return;
    const templates = JSON.parse(localStorage.getItem('victoria_mc_minicourse_bot_templates') || '[]');
    ...
    localStorage.setItem('victoria_mc_minicourse_bot_templates', JSON.stringify(templates));
    return;
  }
...
export async function deleteBotTemplate(templateId: string): Promise<void> {
  if (IS_MOCK_MODE) {
    ...
    localStorage.setItem('victoria_mc_minicourse_bot_templates', JSON.stringify(filtered));
    return;
  }
...
export async function getBroadcasts(): Promise<BotBroadcast[]> {
  if (IS_MOCK_MODE) {
    if (typeof window === 'undefined') return [];
    const raw = JSON.parse(localStorage.getItem('victoria_mc_minicourse_broadcasts') || '[]');
    return raw.map((b: any) => ({ ...b, message_text: normalizeMessageText(b.message_text) }));
  }

  const { data, error } = await supabase!
    .from('victoria_mc_minicourse_broadcasts')
    .select('*')
    .order('created_at', { ascending: false });
...
export async function saveBroadcastRecord(broadcast: Omit<BotBroadcast, 'id' | 'created_at'>): Promise<BotBroadcast> {
  ...
  if (IS_MOCK_MODE) {
    ...
    if (typeof window !== 'undefined') {
      const items = JSON.parse(localStorage.getItem('victoria_mc_minicourse_broadcasts') || '[]');
      items.unshift(item);
      localStorage.setItem('victoria_mc_minicourse_broadcasts', JSON.stringify(items));
    }
    return item;
  }
...
export async function getBotConfig(): Promise<BotConfig> {
  ...
  if (IS_MOCK_MODE) {
    ...
    const saved = localStorage.getItem('victoria_mc_minicourse_bot_config');
    if (saved) return JSON.parse(saved);
    return { id: 'default', is_connected: !!envToken, bot_token: envToken };
  }

  const { data, error } = await supabase!
    .from('victoria_mc_minicourse_bot_config')
    .select('*')
    .eq('id', 'default')
    .maybeSingle();
...
export async function saveBotConfig(config: Partial<BotConfig>): Promise<BotConfig> {
  ...
  if (IS_MOCK_MODE) {
    if (typeof window !== 'undefined') {
      localStorage.setItem('victoria_mc_minicourse_bot_config', JSON.stringify(updated));
    }
    return updated;
  }
